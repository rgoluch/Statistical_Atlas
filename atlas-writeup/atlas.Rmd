...
library(ggmapr)
popmap$jitters1 <- round(popmap$Number/50000,0)
popmap$jitters2 <- round((popmap$Number %% 50000)/5000,0)
popmap$jitters3 <- round((popmap$Number %% 5000)/500,0)
popmap %>% filter(!is.na(Number)) %>% ggplot(aes(x = long, y = lat)) +
  ...
  theme(legend.position="top") +
  scale_size_manual("Population:", labels=c("50000","5000", "500"), values=c(1,.5,.25))
```
...
Figure \ref{fig:comparison} shows that the numbers from NT 14 and the microsample closely matches the information on plate #32.
