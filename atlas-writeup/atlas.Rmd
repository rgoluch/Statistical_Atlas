...
```{r echo = FALSE, message=FALSE, warning = FALSE, fig.pos="center"}
library(RColorBrewer)
cols = brewer.pal(n=12, name="Paired")
church <- read.csv("../Data/denominations-1874.csv")
cl <- church %>% gather(key="Denomination", value = "Number", 7:25)
library(ggmosaic)
cl <- cl %>% mutate(Denomination = reorder(Denomination, Number, na.rm=TRUE))

cl %>%
  ggplot() +
  geom_bar(aes(weight = Number,  x=State.Name, fill=Denomination),
           position="fill") +
