...
Of particular interest are plates #31 and #32. Both of these plates have a very similar studture: they show what are now known as mosaic plots or  Marimekko charts in small multiples, one for each state. 
...
Data retrieved from @NHGIS - from table NT13 on "Employed Population by Occupation by Age by Sex". XXX The numbers in this table are state-level aggregates of population numbers by gender and occupation. XX Adding sample?
Because state-level aggregates of the number of total population above the age of ten is not available, we are making use of the 1\% microsample of the ninth census provided by the Integrated Public Use Microdata Series (IPUMS-USA) provided through the Minnesota Population Center [@ipums]. This allows us to find state-wide estimates for the size of the male and female population above ten for each state. Based on these estimates we derive the size of the population not "gainfully employed" or attending school. 
