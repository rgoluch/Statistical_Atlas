...
author:
  ...
  - name: Susan Vander Plas
    email: srvanderplas@mail.iastate.edu
    affiliation: Iowa1
  - name: Ryan Goluch
...
XXX SVP: I'm sure there are better reproducible research references. I don't know about citing the knitr book alone (don't want to be tool specific) but I've added a couple of placeholder references.      
XXX HH: we probably should include Sweave as well.     
XXX SVP: added sweave reference    
...
![Zoom-in to the legend section of plate #31. On the right is a copy of the colors using a Deuteranopia filter (Color Oracle). The fully saturated red and green are indistinguishable, as are the fully saturated purple (for Roman Catholic) and the brown (for all other denominations). \label{fig:legend-31}](images/plate31-legend.png)
...
colMNSL <- c(colMNSL, darker(colMNSL, step=2), darker(colMNSL, step=4))
colHEX <- mnsl(colMNSL)
colHEX <- colHEX[rep(0:6, each=3)+c(1,8,15)]
colHEX <- c(colHEX, "grey60")

church <- read.csv("../Data/denominations-1874.csv")
cl <- church %>% gather(key="Denomination", value = "Number", c(3:21, 25))
...
```{r echo = FALSE, message=FALSE, warning = FALSE, fig.pos="center", fig.cap="US overview", fig.width=8, fig.height=8}
library(ggmosaic)
...
```{r echo = FALSE, message=FALSE, warning = FALSE, fig.pos="center", fig.height = 8, fig.width=10, fig.cap="Proportion of religious sittings by Denomination"}
...
```{r echo = FALSE, message=FALSE, warning = FALSE, fig.pos="center", fig.height = 8, fig.width=10, fig.cap="Proportion of religious sittings by Denomination including Unaccommodated Population"}
