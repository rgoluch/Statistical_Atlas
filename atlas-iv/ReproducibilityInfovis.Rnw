...
\begin{enumerate}
\item What is computational reproducibility - and how can we implement it
\item Translational reproducibility: using other means to reproduce a particular result. Keith Baggerly uses this approach in what he calls `forensic bioinformatics'.
...
In addition, it is important to recognize the infrastructure that facilitates this study. The Census Bureau has maintained the data from the 1870 census and IPUMS preserves a 1\% sample of that data, made available online today. The Library of Congress maintains high-resolution scans of the original statistical atlas, which allows us to examine the fine details of the original prints. Reproducibility depends on openness: open data and open publications. Without both repositories of information, it would be difficult or impossible to go back and compare past data visualizations with todays methodology.
