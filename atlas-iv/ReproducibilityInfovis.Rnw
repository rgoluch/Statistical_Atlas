...
<<plate32-scatterplot, include=FALSE, results='hide', echo=FALSE, fig.pos="center", fig.width = 7.5, fig.height = 5, warning = FALSE, fig.cap="Set of scatterplots showing a comparison of estimates of occupation percentages based on the Census Data (y) and Chart measurements (x). ", warning=FALSE, message=FALSE>>=
# only run this once
...
Let $n_{UA}^{(\text{XY})}$ be the --unfortunately unkown-- size of the unaccounted population of state {\it XY}. Then we have the relationship
...
<<overall-work-plot, eval=FALSE, out.width='.7\\columnwidth', fig.width=5, fig.height=5.5, echo = FALSE, message=FALSE, fig.pos="center", fig.cap="Recreation of the mosaicplot based on gainfully employed population over ten.\\label{fig:us-plate32-redo}">>=
...
<<state-work-mosaics, include=FALSE, fig.width=10, fig.height=12, echo = FALSE, message=FALSE, fig.pos="center", fig.cap="Recreation of mosaics of gainful occupation by states.">>=
...
On another note: the Census information about territories is available at a higher resolution than shown on plate \#32. Charts with mosaicplots of all territories are available in the online supplement.
...
\item[\bf Re-ordering:] Within each state denominations are ordered from largest to smallest. This makes comparisons of the state-wide religious makeup between states  cognitively more difficult, as it re-orders the colors in the barcode-like color strips representing each state and small differences in numbers might result in rather large visual differences.
\end{compactitem}
%XXX next sentence is a bit strange XXX Due to the decision to show only the four most common denominations for each state, denominations such as Judaism which have a small proportion in each state (but have an overall larger population) do not appear in any state plot, while denominations which are extremely concentrated in some states (such as the Mormon church in Utah) but have fewer members overall are shown. In addition, as denominations are ordered from largest to smallest within each state, making cross-state comparisons difficult, as the denominations are in a different order in each sub-plot.
...
\caption{Church sittings by denomination. \label{fig:barchart-31}}
 \end{subfigure}
\caption{Old and new side-by-side: XXX color scheme XXX. On the right we see that the number of unaccommodated people over the age of ten is higher than the number of sittings in any of the denominations.}
...
A close examination of plate \#31 will also reveal that the different color layers of the lithographic prints are not as well aligned. In addition, the state plots are much more variable in size in plate \#31 than in plate \#32. These differences suggest that plate \#31 might have been created by a less experienced lithographer than plate \#32. Both plate \#31 and plate \#32 were based off of data commpiled from the 1870 census by Francis A. Walker, but it appears the artistry and methodology may not have been consistent between the two plates.
...
\section{Discussion}
Plates \# 31 and \# 32 of the statistical atlas are reproducible using modern methods and a bit of exploratory analysis. This reproducibility is only possible because of the infrastructure created by the US Census bureau, the only government agency with constitutional mandate to gather data, as described in Article 1, Section 2:
\begin{quote}
...
Mosaic plots only feature in this Statistical Atas. In the Atlas accompanying the 1880 US decennial survey they are replaced by pie charts (link to the same theme chart with pies?).
}
...
\meh{I don't think that last part has changed much :(}
