...
\item[\bf Re-ordering:] Within each state denominations are ordered from largest to smallest. This makes comparisons of the state-wide religious makeup between states  cognitively more difficult, as it re-orders the colors in the barcode-like color strips representing each state.
\end{compactitem}
...
\begin{figure}
\centering
\includegraphics[width=.7\columnwidth]{images/plate31-legend.png}
...
A close examination of plate \#31 will also reveal that the different color layers of the lithographic prints are not as well aligned. In addition, the state plots are much more variable in size in plate \#31 than in plate \#32. These differences suggest that plate \#31 might have been created by a less experienced lithographer than plate \#32.
