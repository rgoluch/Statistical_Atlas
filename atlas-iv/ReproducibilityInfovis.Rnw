...
<<barchart-31, echo = FALSE, fig.width=5, fig.height=5, fig.cap="Some text\\label{fig:barchart-31}">>=
cl %>% ggplot(aes(x = Denomination, weight=Number, fill=Denomination)) +
  geom_bar() + theme_bw() + coord_flip() +
  scale_fill_manual(values=rev(colHEX)[-c(3:4)]) +
  theme(legend.position="none") +
  annotate(geom="text", x = 1:20, y=6.5e+6, label=levels(cl$Denomination),
           hjust=1, size=3, colour = "grey30") +
  ylab("Number of nation-wide sittings") +
