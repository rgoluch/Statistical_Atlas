...
The inside squares of plate~\#31 are rotated stacked barcharts or spine plots \cite{hummel:1996}. Due to the decision to show only the four most common denominations for each state, denominations such as Judaism which are a small proportion of each state (but have an overall larger population) do not appear in any state plot, while denominations which are extremely concentrated in a single state (such as Mormons in Utah) but have fewer members overall are shown. In addition, as denominations are ordered from largest to smallest within each state, making cross-state comparisons is difficult, as the denominations are in a different order in each sub-plot.
